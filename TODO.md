...
**第 6 步：提交 Sitemap**
1. 左侧菜单点「Sitemap」
2. 输入框填：`sitemap_index.xml`（一次提交即可收录全站所有文章：VPN、AI、Apple）
3. 点「提交」，状态显示「成功」即可

> ⚠️ **若子 sitemap 显示「无法抓取」**：GitHub Pages 对 XML 文件可能返回错误 Content-Type，导致 GSC 无法解析。**备用方案**：提交 HTML 站点地图 `sitemap/`，或使用「检查网址」逐个请求收录重要页面。首页已添加「站点地图」链接，Google 可从中发现全站链接。
...
- [ ] ⬜ **Google Search Console 验证域名**（详细步骤见第 6 节第二部分）
  ...
  4. 回到 GSC 点「验证」
- [ ] ⬜ **提交 Sitemap**：GSC 左侧「Sitemap」→ 输入 `sitemap_index.xml` → 提交
...
| Sitemap Index | `https://wordok.top/sitemap_index.xml` |
